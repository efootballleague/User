...
var BADGES = {
  ...
  unbeaten:     { icon:'🛡️', label:'Unbeaten Run',     color:'#00FF85', desc:'5+ games without a loss' },
  top_form:     { icon:'🔥', label:'On Fire',          color:'#FF8C00', desc:'Won last 3 games in a row' },
  clean_sheet:  { icon:'🧤', label:'Clean Sheet King', color:'#00d4ff', desc:'3+ clean sheets in last 10' },
  giant_killer: { icon:'⚡', label:'Giant Killer',     color:'#FF2882', desc:'Beat a top 3 player' },
...
function showBroadcastBanner(b){
  var colors={info:{bg:'rgba(0,212,255,0.1)',border:'#00d4ff',icon:'📢'},warning:{bg:'rgba(255,107,0,0.1)',border:'#FF8C00',icon:'⚠️'},celebration:{bg:'rgba(255,230,0,0.1)',border:'#FFE600',icon:'🎉'}};
...
function renderH2H(uid1,uid2){
  ...
  var html='<div style="background:var(--card2);border-radius:12px;padding:.9rem;margin-bottom:.7rem">'
    ...
    +'<div style="text-align:center;flex:1"><div style="font-family:Orbitron,sans-serif;font-weight:900;font-size:1.6rem;color:#00FF85">'+w1+'</div><div style="font-size:.6rem;color:var(--dim)">'+esc(p1.username)+' Wins</div></div>'
  ...
  meetings.slice(0,5).forEach(function(m){
    ...
    var r=g1>g2?'W':g1<g2?'L':'D',rc=r==='W'?'#00FF85':r==='L'?'#FF2882':'#FFE600';
    html+='<div style="display:flex;align-items:center;gap:.55rem;padding:.4rem .55rem;background:rgba(0,0,0,0.35);border-radius:8px;margin-bottom:3px">'
...
function renderFormChart(uid){
  ...
  var bars=results.map(function(r){
    var c=r.r==='W'?'#00FF85':r.r==='D'?'#FFE600':'#FF2882';
...
function renderLeaderboard(){
  ...
  el.innerHTML='<div class="sh"><div class="st" style="color:#FFE600">🏅 Leaderboard</div><div class="sl" style="background:linear-gradient(90deg,#ffe600,transparent)"></div></div>'
    ...
    +makeTable('🏆 MOST WINS','#00FF85',byWins,'w',' wins')
    +makeTable('🔥 LONGEST STREAK','#FF8C00',byStreak,'streak',' in a row')
...
var RESTRICTION_TYPES={
  ...
  no_chat:    {label:'Cannot use chat',        icon:'💬', color:'#FF8C00'},
