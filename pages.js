...
function renderRecentRes(){
  ...
  el.innerHTML=played.map(function(m){
    ...
    return'<div class="card" style="padding:.85rem;cursor:pointer" onclick="openUserModal(\''+hp.uid+'\')">'
      ...
      +'<div style="background:rgba(0,0,0,0.35);border-radius:7px;padding:5px 10px;text-align:center">'
...
function renderFx(){
  ...
  order.forEach(function(dk,gi){
    ...
    groups[dk].forEach(function(m){
      ...
      var statusC=m.played?'#00FF85':isPost?'#FF8C00':'var(--dim)';
      var isPending=m.pendingResult&&!m.played;
    var statusT=m.played?'FT':isPending?('⏳ Ref Review'):(isPost?'POSTPONED':''+timeStr);
    var statusC=m.played?'#00FF85':isPending?'#00d4ff':isPost?'#FF8C00':'var(--dim)';
      var score=m.played
        ...
        :'<div style="font-size:.62rem;color:'+(isPost?'#FF8C00':'var(--dim)')+';font-weight:700">'+(isPost?'PST':'vs')+'</div>';
      html+='<div class="card" style="padding:.85rem;margin-bottom:.45rem;'+(isMine?'border-color:rgba(255,230,0,0.28);':isPost?'border-color:rgba(255,107,0,0.28);opacity:.8;':'')+'">'
        ...
        +(m.refStatus==='rejected'?'<span style="font-size:.54rem;font-weight:700;color:#FF2882;background:rgba(255,0,110,0.08);border:1px solid rgba(255,0,110,0.2);border-radius:8px;padding:1px 5px">&#10060; REJECTED</span>':'')
        ...
        +'<div style="background:rgba(0,0,0,0.35);border-radius:7px;padding:5px 9px;text-align:center;min-width:50px">'+score+'</div>'
